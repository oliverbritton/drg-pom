...
#define nrn_init _nrn_init__nav19hw
#define _nrn_initial _nrn_initial__nav19hw
#define nrn_cur _nrn_cur__nav19hw
#define _nrn_current _nrn_current__nav19hw
#define nrn_jacob _nrn_jacob__nav19hw
#define nrn_state _nrn_state__nav19hw
#define _net_receive _net_receive__nav19hw 
#define _f_rates _f_rates__nav19hw 
#define rates rates__nav19hw 
#define states states__nav19hw 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_nav19hw", _hoc_setdata,
 "rates_nav19hw", _hoc_rates,
 0, 0
};
 /* declare global and static user variables */
#define htau htau_nav19hw
 double htau = 0;
#define hinf hinf_nav19hw
 double hinf = 0;
#define mtau mtau_nav19hw
 double mtau = 0;
#define minf minf_nav19hw
 double minf = 0;
#define stau stau_nav19hw
 double stau = 0;
#define sinf sinf_nav19hw
 double sinf = 0;
#define usetable usetable_nav19hw
 double usetable = 1;
 /* some parameters have upper and lower limits */
 static HocParmLimits _hoc_parm_limits[] = {
 "gbar_nav19hw", 0, 1e+009,
 "usetable_nav19hw", 0, 1,
 0,0,0
};
 ...
 "minf_nav19hw", &minf_nav19hw,
 "hinf_nav19hw", &hinf_nav19hw,
 "sinf_nav19hw", &sinf_nav19hw,
 "mtau_nav19hw", &mtau_nav19hw,
 "htau_nav19hw", &htau_nav19hw,
 "stau_nav19hw", &stau_nav19hw,
 "usetable_nav19hw", &usetable_nav19hw,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 nav19hw F:/CLPC48/drg-pom/drgpom/models/Nav19hw.mod\n");
...
static int _reset;
static char *modelname = "Nav 1.9 from Huang Waxman";
...
static int  _f_rates (  double _lv ) {
  ...
  _lalpha_m = 0.751 / ( 1.0 + exp ( - ( _lv + 32.26 ) / 13.71 ) ) ;
   _lbeta_m = 5.68 / ( 1.0 + exp ( ( _lv + 123.71 ) / 13.94 ) ) ;
   minf = _lalpha_m / ( _lalpha_m + _lbeta_m ) ;
   mtau = 1.0 / ( _lalpha_m + _lbeta_m ) ;
   _lalpha_h = 0.082 / ( 1.0 + exp ( ( _lv + 113.69 ) / 17.4 ) ) ;
   _lbeta_h = 0.24 / ( 1.0 + exp ( - ( _lv - 10.1 ) / 17.2 ) ) ;
   hinf = _lalpha_h / ( _lalpha_h + _lbeta_h ) ;
   htau = 1.0 / ( _lalpha_h + _lbeta_h ) ;
   _lalpha_s = 0.019 / ( 1.0 + exp ( ( _lv + 154.51 ) / 11.46 ) ) ;
   _lbeta_s = 0.000376 / ( 1.0 + exp ( - ( _lv + 60.92 ) / 15.79 ) ) ;
 ...
 v=_v;
{
  ena = _ion_ena;
 { error =  states();
 if(error){fprintf(stderr,"at line 46 in file Nav19hw.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "Nav19hw.mod";
static const char* nmodl_file_text = 
