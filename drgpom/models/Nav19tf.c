...
#define nrn_init _nrn_init__nav19tf
#define _nrn_initial _nrn_initial__nav19tf
#define nrn_cur _nrn_cur__nav19tf
#define _nrn_current _nrn_current__nav19tf
#define nrn_jacob _nrn_jacob__nav19tf
#define nrn_state _nrn_state__nav19tf
#define _net_receive _net_receive__nav19tf 
#define _f_rates _f_rates__nav19tf 
#define rates rates__nav19tf 
#define states states__nav19tf 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_nav19tf", _hoc_setdata,
 "rates_nav19tf", _hoc_rates,
 0, 0
};
 /* declare global and static user variables */
#define htau htau_nav19tf
 double htau = 0;
#define hinf hinf_nav19tf
 double hinf = 0;
#define mtau mtau_nav19tf
 double mtau = 0;
#define minf minf_nav19tf
 double minf = 0;
#define stau stau_nav19tf
 double stau = 0;
#define sinf sinf_nav19tf
 double sinf = 0;
#define usetable usetable_nav19tf
 double usetable = 1;
 ...
 "minf_nav19tf", &minf_nav19tf,
 "hinf_nav19tf", &hinf_nav19tf,
 "sinf_nav19tf", &sinf_nav19tf,
 "mtau_nav19tf", &mtau_nav19tf,
 "htau_nav19tf", &htau_nav19tf,
 "stau_nav19tf", &stau_nav19tf,
 "usetable_nav19tf", &usetable_nav19tf,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 nav19tf F:/CLPC48/drg-pom/drgpom/models/Nav19tf.mod\n");
...
static int _reset;
static char *modelname = "Nav 1.9 from Tigerholm ";
...
static int  _f_rates (  double _lv ) {
   double _lalpha_m , _lbeta_m , _lalpha_h , _lbeta_h , _lalpha_s , _lbeta_s ;
  _zq10 = 2.5 ;
   _lalpha_m = 1.032 / ( 1.0 + exp ( ( _lv + 6.99 ) / ( 14.87115 ) ) ) ;
   _lbeta_m = 5.79 / ( 1.0 + exp ( ( _lv + 130.4 ) / 22.9 ) ) ;
   minf = _lalpha_m / ( _lalpha_m + _lbeta_m ) * _zq10 ;
   mtau = 1.0 / ( _lalpha_m + _lbeta_m ) * _zq10 ;
   _lalpha_h = 0.38685 / ( 1.0 + exp ( ( _lv + 122.35 ) / 15.29 ) ) ;
   _lbeta_h = - 0.00283 + 2.00283 / ( 1.0 + exp ( - ( _lv + 5.5266 ) / 12.70195 ) ) ;
   hinf = _lalpha_h / ( _lalpha_h + _lbeta_h ) ;
   htau = 1.0 / ( _lalpha_h + _lbeta_h ) ;
   _lalpha_s = 0.00003 + 0.00092 / ( 1.0 + exp ( ( _lv + 93.9 ) / 16.6 ) ) ;
   _lbeta_s = 132.05 - 132.05 / ( 1.0 + exp ( ( _lv - 384.9 ) / 28.5 ) ) ;
   sinf = _lalpha_s / ( _lalpha_s + _lbeta_s ) ;
 ...
 v=_v;
{
  ena = _ion_ena;
 { error =  states();
 if(error){fprintf(stderr,"at line 45 in file Nav19tf.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "Nav19tf.mod";
static const char* nmodl_file_text = 
