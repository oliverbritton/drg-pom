...
#define nrn_init _nrn_init__kacw
#define _nrn_initial _nrn_initial__kacw
#define nrn_cur _nrn_cur__kacw
#define _nrn_current _nrn_current__kacw
#define nrn_jacob _nrn_jacob__kacw
#define nrn_state _nrn_state__kacw
#define _net_receive _net_receive__kacw 
#define _f_rates _f_rates__kacw 
#define rates rates__kacw 
#define states states__kacw 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_kacw", _hoc_setdata,
 "htautrap_kacw", _hoc_htautrap,
 "rates_kacw", _hoc_rates,
 0, 0
};
#define htautrap htautrap_kacw
 extern double htautrap( double );
 /* declare global and static user variables */
#define htau htau_kacw
 double htau = 0;
#define hinf hinf_kacw
 double hinf = 0;
#define ntau ntau_kacw
 double ntau = 0;
#define ninf ninf_kacw
 double ninf = 0;
#define usetable usetable_kacw
 double usetable = 1;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 kacw F:/CLPC48/drg-pom/drgpom/models/Kacw.mod\n");
...
static int _reset;
static char *modelname = "IKA from Choi/Waxman";
...
static int  _f_rates (  double _lv ) {
    ninf = pow( ( 1.0 / ( 1.0 + exp ( - ( _lv + 5.4 ) / 16.4 ) ) ) , 4.0 ) ;
   ntau = 0.25 + 10.04 * exp ( ( - pow( ( _lv + 24.67 ) , 2.0 ) ) / ( 2.0 * pow( 34.8 , 2.0 ) ) ) ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lv + 49.9 ) / 4.6 ) ) ;
   htau = 20.0 + 50.0 * exp ( ( - pow( ( _lv + 40.0 ) , 2.0 ) ) / ( 2.0 * pow( 40.0 , 2.0 ) ) ) ;
 ...
 v=_v;
{
  ek = _ion_ek;
 { error =  states();
 if(error){fprintf(stderr,"at line 45 in file Kacw.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "Kacw.mod";
static const char* nmodl_file_text = 
