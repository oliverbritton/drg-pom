...
#define nrn_init _nrn_init__kdrtf
#define _nrn_initial _nrn_initial__kdrtf
#define nrn_cur _nrn_cur__kdrtf
#define _nrn_current _nrn_current__kdrtf
#define nrn_jacob _nrn_jacob__kdrtf
#define nrn_state _nrn_state__kdrtf
#define _net_receive _net_receive__kdrtf 
#define _f_rates _f_rates__kdrtf 
#define rates rates__kdrtf 
#define states states__kdrtf 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_kdrtf", _hoc_setdata,
 "rates_kdrtf", _hoc_rates,
 "taucalc_kdrtf", _hoc_taucalc,
 0, 0
};
#define taucalc taucalc_kdrtf
 extern double taucalc( double );
 /* declare global and static user variables */
#define ntau ntau_kdrtf
 double ntau = 0;
#define ninf ninf_kdrtf
 double ninf = 0;
#define usetable usetable_kdrtf
 double usetable = 1;
 ...
 "ninf_kdrtf", &ninf_kdrtf,
 "ntau_kdrtf", &ntau_kdrtf,
 "usetable_kdrtf", &usetable_kdrtf,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 kdrtf F:/CLPC48/drg-pom/drgpom/models/Kdrtf.mod\n");
...
static int _reset;
static char *modelname = "IKdr from Tigerholm 2014";
...
double taucalc (  double _lx ) {
   double _ltaucalc;
 if ( _lx > - 31.0 ) {
     _ltaucalc = 0.16 + 0.8 * exp ( - 0.0267 * ( _lx + 11.0 ) ) ;
     }
   else {
     _ltaucalc = 1000.0 * ( 0.000688 + 1.0 / ( exp ( ( _lx + 75.2 ) / 6.5 ) + exp ( - ( _lx - 131.5 ) / ( 34.8 ) ) ) ) ;
     }
 ...
 v=_v;
{
  ek = _ion_ek;
 { error =  states();
 if(error){fprintf(stderr,"at line 45 in file Kdrtf.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
...
#if NMODL_TEXT
static const char* nmodl_filename = "Kdrtf.mod";
static const char* nmodl_file_text = 
