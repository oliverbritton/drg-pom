...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 ls /cygdrive/e/CLPC48/Neuroscience Project/Code/Models/Kovalsky/DRG_Devor/late2.mod\n");
...
static int _reset;
static char *modelname = "late2.mod   ttx-sensitive late sodium channel";
...
static int  _f_rates (  double _lv ) {
    minf = 1.0 / ( 1.0 + exp ( ( - 51.8 - _lv ) / 4.6 ) ) ;
   htau = 1.0 / ( 0.04 * exp ( _lv / 25.5 ) ) + 63.2 ;
   hinf = 0.9827 / ( 1.0 + exp ( - ( ( _lv + 55.67 ) / - 6.552 ) ) ) ;
    return 0; }
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 39 in file late2.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
