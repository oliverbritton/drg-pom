...
extern Memb_func* memb_func;
 ...
 "setdata_inter", _hoc_setdata,
 "rates_inter", _hoc_rates,
 "vtrap_inter", _hoc_vtrap,
 0, 0
};
#define vtrap vtrap_inter
 extern double vtrap( double , double );
 /* declare global and static user variables */
#define htau htau_inter
 double htau = 0;
#define hinf hinf_inter
 double hinf = 0;
#define mtau mtau_inter
 double mtau = 0;
#define minf minf_inter
 double minf = 0;
#define usetable usetable_inter
 double usetable = 1;
 /* some parameters have upper and lower limits */
 static HocParmLimits _hoc_parm_limits[] = {
 "gnabar_inter", 0, 1e+09,
 "usetable_inter", 0, 1,
 0,0,0
};
 ...
 "minf_inter", &minf_inter,
 "hinf_inter", &hinf_inter,
 "mtau_inter", &mtau_inter,
 "htau_inter", &htau_inter,
 "usetable_inter", &usetable_inter,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 inter /cygdrive/e/CLPC48/Neuroscience Project/Code/Models/Kovalsky/DRG_Devor/inter.mod\n");
...
static int _reset;
static char *modelname = "inter.mod";
...
static int  _f_rates (  double _lv ) {
  ...
  _zq10 = pow( 3.0 , ( ( celsius - 6.3 ) / 10.0 ) ) ;
   _lalpha = .1 * vtrap ( _threadargscomma_ - ( _lv + 25.0 ) , 10.0 ) ;
   _lbeta = 4.0 * exp ( - ( _lv + 50.0 ) / 18.0 ) ;
   _lsum = _lalpha + _lbeta ;
   minf = _lalpha / _lsum ;
   htau = 0.2218 * exp ( - 0.06883 * _lv ) ;
   hinf = pow( ( 1.0 + exp ( ( _lv + 72.5 ) / 8.0 ) ) , - 1.0 ) ;
    return 0; }
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 42 in file inter.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
